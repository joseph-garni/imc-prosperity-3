...
class Trader:
    def __init__(self):
        ...
        self.csi = 50.0  # Critical Sunlight Index
    ...
    def trade_magnificent_macarons(self, order_depth, state, conv_obs):
        orders = []
        product = 'MAGNIFICENT_MACARONS'
        current_position = self.get_current_position(state, product)
        max_position = self.position_limits[product]
        
        # Extract current sunlight index and sugar price
        ...
        if len(self.sunlight_history) > 100:
            self.sunlight_history.pop(0)
            self.sugar_price_history.pop(0)
            if len(self.macaron_price_history) > 100:
                self.macaron_price_history.pop(0)
        
        # Trading strategy based on sunlight index relative to CSI
        if sunlight_index < self.csi:
            # Below CSI - prices likely to rise
            ...
            # Above CSI - prices likely to stabilize or fall
    ...
    def should_convert_macarons(self, state, current_position):
        # Skip if position is small or cooldown period hasn't elapsed
        if abs(current_position) < 10 or state.timestamp - self.last_conversion_timestamp < self.conversion_cooldown:
            return 0
        
        # Long position conversion
        if current_position > 20:
            self.last_conversion_timestamp = state.timestamp
            return min(current_position, 10)  # Limit to 10 units
        
        # Short position conversion
        if current_position < -20:
            self.last_conversion_timestamp = state.timestamp
            return max(current_position, -10)  # Limit to -10 units
        ...
        for product in state.order_depths:
            ...
            if product == 'MAGNIFICENT_MACARONS':
                if product in state.observations:
                    conv_obs = state.observations[product]
                    orders = self.trade_magnificent_macarons(order_depth, state, conv_obs)

...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if state.traderData:
            try:
                ...
                if isinstance(td, dict):
                    ...
                    self.volcanic_rock_recent_pnl = td.get(
                        "volcanic_rock_recent_pnl",
                        getattr(self, 'volcanic_rock_recent_pnl', [])
                    )
                    self.max_consecutive_losses = td.get(
                        "max_consecutive_losses",
                        getattr(self, 'max_consecutive_losses', 0)
                    )
                    self.last_volcanic_position = td.get(
                        "last_volcanic_position",
                        getattr(self, 'last_volcanic_position', 0)
                    )
            except Exception:
                pass
        ...
        for product, order_depth in state.order_depths.items():
            ...
            self.position_history.setdefault(product, []).append(current_position)
            if len(self.position_history[product]) > self.max_history_size:
                self.position_history[product].pop(0)

            # Track volcanic position change
            if product == 'VOLCANIC_ROCK':
                if hasattr(self, 'last_volcanic_position'):
                    self.volcanic_position_change = current_position - self.last_volcanic_position
                self.last_volcanic_position = current_position

            if mid is not None:
                mid_prices[product] = mid
                self.update_price_history(product, mid, state.timestamp)

        # === Calculate basket & voucher theoretical prices ===
        basket_values = self.calculate_basket_values(mid_prices)
        theoretical_vouchers = {}
        if 'VOLCANIC_ROCK' in mid_prices:
            volcanic_price = mid_prices['VOLCANIC_ROCK']
            theoretical_vouchers = self.calculate_theoretical_voucher_prices(volcanic_price)
            voucher_mid = {
                p: mid_prices[p]
                for p in mid_prices
                if p.startswith('VOLCANIC_ROCK_VOUCHER')
            }
            self.update_implied_vols(volcanic_price, voucher_mid, state.timestamp)
        ...
        if obs and hasattr(obs, 'MAGNIFICENT_MACARONS') and obs.MAGNIFICENT_MACARONS:
            mac_obs = obs.MAGNIFICENT_MACARONS
            macaron_sunlight = getattr(mac_obs, 'sunlightIndex', 0)
            macaron_sugar = getattr(mac_obs, 'sugarPrice', 0)

        # === Track previous PnL for VOLCANIC_ROCK ===
        prev_pnl = getattr(state, 'profit_and_loss', {})
        if 'VOLCANIC_ROCK' in prev_pnl:
            rock_pnl = prev_pnl['VOLCANIC_ROCK']
        ...
        # === Apply risk scaling after consecutive losses ===
        volcanic_risk_limit = 1.0
        rf = self.risk_factors.get('VOLCANIC_ROCK', {})
        if self.max_consecutive_losses >= rf.get('consecutive_loss_threshold', float('inf')):
            volcanic_risk_limit = rf.get('post_loss_scaling', 1.0)
        ...
        for product, order_depth in state.order_depths.items():
            if product == 'MAGNIFICENT_MACARONS':
                orders = self.trade_magnificent_macarons(
                    order_depth, state, macaron_sunlight, macaron_sugar
                )
                conv = self.should_convert_macarons(
                    state, state.position.get(product, 0)
                )
                ...
                orders = self.trade_basket_arbitrage(
                    product, order_depth, state,
                    mid_prices[product], basket_values
                )

            elif product.startswith('VOLCANIC_ROCK_VOUCHER'):
                orders = self.trade_volcanic_rock_vouchers(
                    product, order_depth, state,
                    mid_prices[product], theoretical_vouchers
                )
                ...
                if volcanic_risk_limit < 1.0:
                    limited = OrderDepth()
                    limited.buy_orders = {
                        price: int(vol * volcanic_risk_limit)
                    ...
                    od = limited
                orders = self.trade_volcanic_rock(
                    product, od, state, mid_prices[product]
                )

            elif product == 'SQUID_INK':
                orders = self.trade_based_on_moving_average(
                    product, order_depth, state, mid_prices[product]
                )
            else:
                orders = self.trade_based_on_moving_average(
                    product, order_depth, state, mid_prices[product]
